...
import static com.bytedance.club.publics.Constant.IS_AUTO_BUY;
import static com.bytedance.club.publics.Constant.KEY_MONEY;
import static com.bytedance.club.publics.Constant.KEY_VOUCHER;
...
import com.bytedance.club.amodel.TaskType;
...
import com.bytedance.club.amodel.bean.ChapterContentSellBean;
import com.bytedance.club.amodel.bean.ChapterItemBean;
import com.bytedance.club.amodel.bean.UserFinanceBean;
import com.bytedance.club.amodel.packges.BookAutoPayTaskPackage;
import com.bytedance.club.amodel.packges.BookAutoPayTaskResult;
import com.bytedance.club.amodel.packges.BookBuyMultiPackage;
import com.bytedance.club.amodel.packges.BookBuyMultiResult;
import com.bytedance.club.amodel.packges.BookBuySinglePackage;
import com.bytedance.club.amodel.packges.BookBuySingleResult;
...
import com.bytedance.club.base.ui.RxPresenter;
...
import com.bytedance.club.datautils.down.NewMultiDownloadManager;
...
import com.bytedance.club.utils.DF;
                ...
                .subscribe(
                        (BookChapterPackage chapterInfo) -> {
                            if (chapterInfo.getResult() != null) {
                                ...
                                LogUtils.d("阅读页目录 ====== " + result.getCatalog().size());
...
//                                mView.showCategory(finalTempLastChapterArray);
                                ...
                                LogUtils.d("阅读页目录 失败 ====== " + chapterInfo.reason);
                                mView.showCategory(finalTempLastChapterArray);
                            }
...
//
                                ...
                                LogUtils.e("oss 请求 =====" + chapter.getTitle());
                                LogUtils.e("oss 请求 =====" + contentStr);
                                        ...
                                        case "SN031"://未购买
                                        {
                                            ...
                                            if (sellBean != null) {
                                                if (userFinanceBean != null) {
                                                    ...
                                                    if (user != null) {
                                                        SharedPreferencesUtil.putInt(user.config, KEY_MONEY, user.money);
                                                        SharedPreferencesUtil.putInt(user.config, KEY_VOUCHER, user.voucher);
...
//                            finishChapter();
                            ...
                            LogUtils.e("内容请求 ===== " + chapter.getTitle());
                        },
                        (e) -> {
                            LogUtils.e("内容错误=====" + e);
                            ChapterPageStatusInfo statusInfo = new ChapterPageStatusInfo();
    ...
    public void buySingleChapter(String bookId, TxtChapter chapter) {
        ...
        LogUtils.e("章节购买 ====== " + chapter.getTitle());
...
//                                    downChapterContents(chapter);
                                    ...
                                    LogUtils.e("单张购买 失败错误=====" + result.msg);
                        ...
                        (BookBuyMultiPackage buyMultiPackageResult) -> {
                            ...
                            if (buyMultiPackageResult != null && buyMultiPackageResult.getResult() != null) {
                                ...
                                if (result.status.equals("1")) {
                                    ...
                                    LogUtils.e("批量购买 失败错误=====" + result.msg);

                                    mView.buyErrorChapter(chapter);
                                }
                            } else {
                                LogUtils.e("批量购买 失败错误===== 222");
                                mView.buyErrorChapter(chapter);
                            }
                        },
                        (e) -> {
                            LogUtils.e("批量购买 失败错误=====" + e);
                            mView.buyErrorChapter(chapter);
                        }
                );
        addDisposable(disposable);
        ...
        Disposable chapterDispo = ReaderRemoteRepository.getInstance()
                ...
                .subscribe(
                        responseBody -> {
                            ...
                            LogUtils.d("oss 阅读页内容 ====== " + chapter.getTitle() + " ======== " + contentStr);
                        ...
                        (e) -> {
                            LogUtils.e(" oss 阅读页内容  加载失败====== " + e + " ==== " + chapter.getTitle());
    ...
    public void getMoreBuyMultiInfo(String bookId, TxtChapter chapter) {
        ...
        Disposable disposable = ReaderRemoteRepository.getInstance()
                ...
                .subscribe(
                        ...
                        (e) -> {
                            LogUtils.e("批量购买内容  加载失败====== " + e + " ==== " + chapter.getTitle());
                        }
                );
    ...
    public void getBookAutoTaskInfo(String bookId) {
        Disposable disposable = ReaderRemoteRepository.getInstance()
                ...
                .subscribe(
                        ...
                        (e) -> {
                            LogUtils.e("自动购买人物信息  加载失败====== " + e + " ==== " + bookId);
                        }
                );
    ...
    public void getBookCommentList(String bookId) {
        ...
        Disposable disposable = ReaderRemoteRepository.getInstance()
                .ReaderBookCommentList(bookId, 1, ONE)
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .subscribe(
