...
import static com.bytedance.club.api.NetRequest.base64;
import static com.bytedance.club.homepage.HomeActivity.contactList;
import static com.bytedance.club.homepage.HomeActivity.version_status;
...
import com.bytedance.club.amodel.TopUpListBean;
import com.bytedance.club.amodel.TopUpListBean.ResultData;
import com.bytedance.club.api.API;
import com.bytedance.club.api.NetRequest;
import com.bytedance.club.booktopup.adapter.TopUpAdapter;
import com.bytedance.club.person.landing.LoginActivity;
import com.bytedance.club.person.pay.PaymentUtil;
import com.bytedance.club.person.personcenter.FeedbackActivity;
import com.bytedance.club.person.personcenter.UserHelpActivity;
...
//    protected aiyeWebWidget1 mWebView;
    ...
    private void loadUrl(TopUpListBean.ResultData.Info.Order_data mOrder) {
        ...
        String url = PlotRead.getINDEX() + NetRequest.path(API.H5_PAY_PAL, param);
            ...
            public void onBillingSetupFinished(BillingResult billingResult) {
                if (billingResult.getResponseCode() == BillingClient.BillingResponseCode.OK) {
                    ...
                    if (contactList == null || contactList.size() == 0) {
                        ...
                        mTopUpAdapter = new TopUpAdapter(context, contactList, onItemClick);
                        mRecyclerView.setAdapter(mTopUpAdapter);
                        mLayoutInfo.setVisibility(View.VISIBLE);
                        if (isConnect) {
                            for (int i = 0; i < contactList.size(); i++) {
                                getSkuList(contactList.get(i).goods, false, i);
                            }
                        }
                    }
                    onPurchasesUpdated();
    ...
    private final OnItemClickListener onItemClick = new OnItemClickListener() {
        ...
        public void onItemClick(RecyclerView.ViewHolder viewHolder) {
            ...
            goodsId = contactList.get(recordPosition).goods;
            amount = contactList.get(recordPosition).rmb;
            ...
            if (version_status == 1) {
                ...
                loadUrl(contactList.get(recordPosition));
                ...
                doPay(contactList.get(recordPosition).rmb, contactList.get(recordPosition).id, 1
                        , 1
                        , contactList.get(recordPosition).note
                        , contactList.get(recordPosition).rmb, null);
            }
            ...
            public void onSuccess(JSONObject data) {
                ...
                if (SN000.equals(serverNo)) {
                    try {
                        ...
                        contactList.clear();
                        Type listType = new TypeToken<List<ResultData.Info.Order_data>>() {
                        }.getType();
                        Gson gson = new Gson();
                ...
                for (Purchase purchase : purchasesResult.getPurchasesList()) {
                    ...
                    if (payinfos.size() == 0) {
                        for (int k = 0; k < contactList.size(); k++) {
                            if (contactList.get(k).goods.equals(purchase.getSku())) {
                                isRestore = false;
                                doPay(contactList.get(k).rmb, contactList.get(k).id, 1
                                        , 1
                                        , contactList.get(k).note
                                        , contactList.get(k).rmb, purchase);
                                break;
                            }
                            ...
                            if (billingResult.getResponseCode() == BillingClient.BillingResponseCode.OK
                                ...
                                if (skuDetailsList.size() > 0) {
                                    ...
                                    if (isPay) {
                                        ...
                                        contactList.get(position).is_rec = skuDetail.getPrice();
                                        if (TextUtils.isEmpty(skuDetail.getPriceCurrencyCode())) {
                                            contactList.get(position).note = skuDetail.getPrice();

                                        } else {
                                            contactList.get(position).note = skuDetail.getPriceCurrencyCode();
                                        }
