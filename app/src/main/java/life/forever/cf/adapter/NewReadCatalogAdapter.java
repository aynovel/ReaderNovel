...
public class NewReadCatalogAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public NewReadCatalogAdapter(Context context, List<ChapterItemBean> Catalogs, List<Integer> hotIds) {
        ...
        this.hotIds = hotIds;
        this.titleColor = Constant.application.getResources().getColor(R.color.color_656667);
    }
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        return new NewReadCatalogAdapter.CatalogViewHolder(LayoutInflater.from(mContext).inflate(R.layout.item_read_catalog, parent, Constant.FALSE));
    }
...
//        viewHolder.status.setImageResource(drawable);
        if (position == currentPosition) {
            viewHolder.title.setTextColor(Constant.THEME_COLOR);
        } else {
            viewHolder.title.setTextColor(titleColor);
        }
